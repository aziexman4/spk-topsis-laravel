            ...
            <div class="bg-white dark:bg-zinc-800 overflow-hidden shadow-xl shadow-zinc-200/40 dark:shadow-none sm:rounded-2xl border border-zinc-200 dark:border-zinc-700/50 p-8">
                ...
                <form action="{{ route('periode.update', $periode->id) }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="nama_periode" id="nama_periode" class="w-full bg-zinc-50 dark:bg-zinc-900/50 border border-zinc-300 dark:border-zinc-700 text-zinc-900 dark:text-white rounded-xl focus:ring-violet-500 focus:border-violet-500 block p-3 transition-colors" required value="{{ old('nama_periode', $periode->nama_periode) }}">
                        @error('nama_periode')
                    ...
                    <div class="mb-8">
                        <label class="inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_active" class="sr-only peer" value="1" {{ $periode->is_active ? 'checked' : '' }}>
                            ...
                            <span class="ms-3 text-sm font-medium text-zinc-700 dark:text-zinc-300">Status Aktif</span>
                        </label>
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-2">Peringatan: Gelombang lama yang diaktifkan kembali akan menonaktifkan gelombang saat ini.</p>
                    </div>
